...
function ResourcesPage() {
  return <main className="min-h-screen bg-gray-950 text-white">
    ...
    <section className="px-6 py-12 sm:py-16"><div className="mx-auto max-w-5xl"><div className="grid gap-5 md:grid-cols-2">{guides.map((guide)=><Link key={guide.to} to={guide.to} className="group rounded-2xl border border-gray-800 bg-gray-900/50 p-6 transition hover:border-indigo-500/60 hover:bg-gray-900"><span className="text-xs font-semibold uppercase tracking-wider text-indigo-400">{guide.tag}</span><h2 className="mt-3 text-xl font-semibold text-white group-hover:text-indigo-200">{guide.title}</h2><p className="mt-3 leading-7 text-gray-400">{guide.description}</p><span className="mt-5 inline-block text-sm font-medium text-indigo-400">Read guide →</span></Link>)}</div>
      <div className="mt-14 rounded-2xl border border-indigo-900/60 bg-indigo-950/20 p-7"><p className="text-sm font-semibold uppercase tracking-wider text-indigo-400">From guide to workflow</p><h2 className="mt-2 text-2xl font-bold">Need more than a PDF?</h2><p className="mt-3 max-w-3xl leading-7 text-gray-300">Use the free scanner for general paperwork, or choose a focused DocSnap module when you want receipts, bills, home records, contracts, books, tools, or meetings organized around the job itself.</p><div className="mt-6 flex flex-wrap gap-3"><Link to="/" hash="modules" className="rounded-lg bg-indigo-600 px-5 py-3 text-sm font-semibold hover:bg-indigo-500">Explore DocSnap modules</Link><Link to="/pricing" className="rounded-lg border border-gray-700 px-5 py-3 text-sm font-semibold text-gray-200 hover:border-gray-500">Compare plans</Link></div></div>
    </div></section>
  </main>;
}